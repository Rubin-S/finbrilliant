    ...
    container.innerHTML = `
      <div class="widget-box p-4 rounded-xl bg-slate-900/90 border border-slate-700/80 shadow-2xl text-slate-100">
        <div class="flex items-center justify-between pb-2 border-b border-slate-800">
          <div>
            <span class="text-xs font-semibold uppercase tracking-wider text-slate-400">DCF Calculator</span>
          ...
          <div class="text-right">
            ...
            <p class="text-base font-extrabold text-white font-mono">$${totalPV.toLocaleString()}</p>
          </div>
        </div>
            ...
            ${bars.map(b => {
              ...
              return `
                <div class="flex flex-col items-center justify-end h-36">
                  <span class="text-[11px] font-bold text-white font-mono mb-1">$${b.pv}</span>
                  <div class="w-full bg-slate-800 rounded-t overflow-hidden flex flex-col justify-end h-28 relative">
                    <div class="w-full bg-white/80 rounded-t transition-all duration-200" style="height: ${heightPct}%"></div>
                  </div>
        ...
        <div class="space-y-2 mt-2 text-xs">
          <div class="flex justify-between">
            <span class="text-slate-300">Discount Rate (WACC): <strong class="text-white font-mono">${discountRate}%</strong></span>
          </div>
          <input type="range" min="4" max="25" step="0.5" value="${discountRate}" class="wacc-slider w-full accent-white cursor-pointer"/>
        </div>
      </div>
    `;
