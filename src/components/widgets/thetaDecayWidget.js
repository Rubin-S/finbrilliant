    ...
    container.innerHTML = `
      <div class="widget-box p-4 rounded-xl bg-slate-900/90 border border-slate-700/80 shadow-2xl text-slate-100">
        <div class="flex items-center justify-between pb-2 border-b border-slate-800">
          <div>
            <span class="text-xs font-semibold uppercase tracking-wider text-slate-400">Options Time Decay</span>
          ...
          <div class="text-right">
            ...
            <p class="text-sm font-bold text-white font-mono">$${currentBS.thetaCall} / day</p>
          </div>
        </div>
        ...
        <div class="my-3 bg-slate-950/70 rounded-lg p-2 border border-slate-800">
          <svg viewBox="0 0 ${width} ${height}" class="w-full h-auto select-none">
            ...
            <!-- 30-day cliff marker -->
            <line x1="${padding.left + (60 / 90) * chartW}" y1="${padding.top}" x2="${padding.left + (60 / 90) * chartW}" y2="${padding.top + chartH}" stroke="#71717a" stroke-dasharray="2 2"/>
            <text x="${padding.left + (60 / 90) * chartW}" y="${padding.top + 10}" fill="#a1a1aa" font-size="9" text-anchor="middle">30-Day Cliff</text>

            <!-- Decay Curve -->
            <polyline fill="none" stroke="#ffffff" stroke-width="3" points="${curvePoints.join(' ')}"/>

            <!-- Current Marker -->
            <circle cx="${curX}" cy="${curY}" r="6" fill="#ffffff" stroke="#000000" stroke-width="2"/>
          ...
          <div class="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-slate-800 text-center text-xs">
            <div class="bg-slate-800/40 p-1.5 rounded">
              ...
              <span class="text-sm font-bold text-white font-mono">$${currentBS.callPrice}</span>
            </div>
            <div class="bg-slate-800/40 p-1.5 rounded">
              <span class="text-[10px] text-slate-400 block">Daily Theta Burn</span>
              <span class="text-sm font-bold text-slate-300 font-mono">${currentBS.thetaCall}</span>
            </div>
            <div class="bg-slate-800/40 p-1.5 rounded">
              <span class="text-[10px] text-slate-400 block">Time to Expiration</span>
              <span class="text-sm font-bold text-white font-mono">${daysLeft} Days</span>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-2 mt-2 text-xs">
          <div class="flex justify-between">
            <span class="text-slate-300">Days to Expiration: <strong class="text-white font-mono">${daysLeft} Days</strong></span>
            <span class="text-slate-400">${daysLeft <= 30 ? 'High Decay Zone (Cliff)' : 'Mild Linear Decay'}</span>
          </div>
          <input type="range" min="1" max="90" step="1" value="${daysLeft}" class="days-slider w-full accent-white cursor-pointer"/>
        </div>
      </div>
    `;
